...
public class AES
{
    ...
    private byte[] Cipher(byte[] toCipher) //deben ser 16 bytes
    {
        ...
        Word[] keyExpanded = KeyExpansion(key);
    ...
    private byte[] WordToByteKey(Word[] key, int round) {
        Log.d("WORD TO BYTE", "INICIO WORD TO BYTE");
        byte[] toReturn = new byte[16];
        Word a = key[round * 4];
        Word b = key[(round * 4) + 1];
        Word c = key[(round * 4) + 2];
        Word d = key[(round * 4) + 3];
        for (int i = 0; i < 4; i++) {
    ...
    private Word[] KeyExpansion(byte[] key) {
        Log.d("KEY EXPANSION", "INICIO");

        Word[] toReturn = new Word[44];
        byte[] Rcon = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, (byte) 0x80, 0x1b, 0x36}; //por alguna razon 0x80 se queja de no ser byte
        for (int i = 0; i < 4; i++) {
            toReturn[i] = new Word(key[4 * i], key[(4 * i) + 1], key[(4 * i) + 2], key[(4 * i) + 3]);
        }

        for (int i = 4; i < 44; i++) {
            Word temp = toReturn[i - 1];
            if (i % 4 == 0) {
                temp.RotWord();
            ...
            toReturn[i] = Word.XOR(toReturn[i - 4], temp);
        }
